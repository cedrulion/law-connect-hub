...
const NewCaseForm = () => {
  ...
  const handleFileChange = (e) => {
    // For this example, we simulate file uploads by storing object URLs
    if (e.target.files) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Create an array of URLs for the files.
      ...
      // Send JSON data; note that we are using "application/json" here.
      ...
      if (response.data.success) {
        // Redirect to the new case details page
        navigate(`/cases/${response.data.data._id}`, {
          state: { message: 'Your case was submitted successfully!' }
        });
      }
    } catch (err) {
      console.error(err);

...
const Message = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      // Assuming fetchMessages will return messages for each client and their length to determine if they've chatted before
  ...
  return (
    <div className="p-6 text-gray-800 bg-gray-100 min-h-screen flex" style={{ fontFamily: 'roboto' }}>
      <div className="w-1/4 bg-white p-6 rounded shadow-md">
        <h2 className="text-xl font-bold mb-4">Messages Center</h2>
        <button
          className="w-full bg-black text-white p-2 rounded shadow-md flex items-center justify-center mb-4"
          onClick={toggleClientView}
        >
          <FaPlus className="mr-2" />
          {viewingNewClients ? 'View Previous Messages' : 'New Message'}
        </button>
        <div>
          <p className="font-bold mb-2">{viewingNewClients ? 'New Clients' : 'Direct Messages'}</p>
          {clients.map((client) => (
            <div
              key={client._id}
              className={`flex items-center p-2 rounded mb-2 cursor-pointer ${
                selectedClient === client._id ? 'bg-gray-200' : 'bg-white'
              }`}
        ...
        {selectedClient ? (
          <div className="flex flex-col h-full">
            ...
            <div className="p-4 flex items-center border-t border-gray-200">
              ...
              />
              <button
                className="ml-2 p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 flex items-center"
              ...
              >
                ...
                >
                  <path
                    ...
                    strokeWidth="2"
                    d="M14.752 11.168l-9.193 3.32a1 1 0 01-1.252-1.252l3.32-9.193A9.995 9.995 0 1121.74 21.74a9.996 9.996 0 01-7.988-10.572z"
                  />
                </svg>
